...
Usage:
  ...
  python scripts/translate.py --messages-dir kj-controller/static-sing/messages --target pt
  ...
  python scripts/translate.py --messages-dir kj-controller/static-sing/messages --target es --full

  # Skip review pass (faster, lower quality)
  python scripts/translate.py --messages-dir kj-controller/static-sing/messages --target es --skip-review
...
## Context
This is for Nomad Karaoke (nomadkaraoke.com). These strings are the singer-facing
"request a song" web page used at live karaoke nights: guests scan a QR code on
the venue screen, type their name, search for a song, and follow their place in
the rotation from their phone. Readers are ordinary bar guests of every age and
background — keep it warm, short, and plain (no slang or idioms that only work
in English). "The host" is the person running the karaoke night (a "KJ").
...
async def review_pass_async(
    ...
    prompt = f"""Review and polish this {target_name} translation of the UI message file for the Nomad Karaoke singer request page (used by bar guests at live karaoke nights).
...
2. Check every string for:
   ...
   - Tone: The tone should be warm, plain and friendly — a karaoke host talking to guests at a bar; avoid idioms that only work in English
