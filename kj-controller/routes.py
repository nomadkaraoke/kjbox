...
# outdoor 100F+). This helper exposes the motherboard ACPI "ambient" zone (a
# room-temperature proxy) that the CPU sensor alone can't tell us about.
#
# The 4TB USB SSD's temperature was also polled here (v0.94.0, via smartctl's
# `sntasmedia` NVMe passthrough) but that was removed in v0.95.0: the
# passthrough admin commands hung the SanDisk's internal ASMedia bridge
# firmware mid-show (drive dropped to "medium not present" / 0 capacity and
# needed a physical replug — see docs/TROUBLESHOOTING.md). Do not reintroduce
# smartctl polling against that drive.
...
def system_stats():
    ...
    try:
        ...
        # Temperature (best-effort — never let a missing sensor break stats).
