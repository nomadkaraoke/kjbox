...
| **karaoke-gen API** | `gen_client.py`, `gen_poller.py` | REST calls with `X-Admin-Token`: `POST /api/audio-search/search` (create job), `GET /api/jobs/{id}`, `GET /api/jobs/{id}/download-urls`, `POST /api/parse-karaoke-titles` (LLM title refine for downloads). The poller maps status to `rotation_entries.gen_status`. On COMPLETE it links the synced NOMAD-#### master (`syncing` until master-sync delivers it; no direct download); `complete_gen_job` links the file and flips `Being Made (!)` → `Waiting`. Singer make-its run as the singer's own gen account via `sing_make.py` (`/api/kjbox/*` partner endpoints + gen customer endpoints). | `gen_api_url`, `gen_api_token`, `gen_poll_interval`, `gen_kjbox_secret`, `sing_make_max_per_device` | Unconfigured: `gen_client`/`gen_poller` are `None` (`app.py:363`, `:555`). The "Make" (gen) request path and LLM refine are then unavailable. Per-poll errors are logged. |
...
- **`approve_sing_request`** (R:5994), by source:
  ...
  - **make:** `add_entry` with status `Being Made (!)`, then `make_jobs.attach_on_approve` binds the gen job `/sing/submit` already created on the singer's account (`sing_make.create_job`). Singer make-its are approved inline at submit (never queue for the KJ).
    - No job on the request (legacy make request): falls back to `gen_client.create_job(artist, title)` (admin-owned); on failure the entry stays unlinked as `Being Made (!)` (approval still succeeds).
- **After approval:**
...
- **Gen completion:** the `GenPoller` thread (every 60s) calls `get_job_status` then `set_gen_status`. On `complete` it links the synced `NOMAD-####` master (waiting as `syncing` until master-sync delivers it — never a direct download); `RM.complete_gen_job` links the file and flips `Being Made (!)` → `Waiting`.
...
| **New Rotation** (index.html:119, with confirm) | `archiveRotation` (8966) | POST `/rotation/archive` | R:`archive_rotation`:3848 → RM.archive_rotation:267 → RS.archive:1234 + RS.clear_history:1511 | Copies all rows → `rotation_archive` (night_date=today), deletes all `rotation_entries`, clears `left_singers_json`, sets `rotation_meta.night_started_at`=now, adds a starter entry "Andrew / First Song of the Night". Clears undo+redo. `SS.set_enabled(True)` | AM (no CK: **not undoable**). The event token is deliberately NOT regenerated. Also starts the `kj-night-capture` recorder sidecar for 12h (`night_capture_launcher.py`, background thread, never fails the archive; see NIGHT-RECORDING.md). The night boundary re-scopes SMS/push phone resolution, photo consent, and alias/rename. Entry ids stay monotonic (sqlite_sequence is not reset) |
...
Every mutation sends `device_id` (since v0.115.0; before that photo-consent and push/subscribe didn't, and a singer tapping consent 11 times in 10 s got 6 × 429 against the **venue-wide per-IP budget**). `tests/unit/test_sing_js_device_id.py` fails on any new singer POST without it. The only exemptions are `/validate` (before the SPA loads) and read-only `/media-info`. Server side, the `singer_aliases` table (`device_id` → `canonical_name`, `origin` `self`|`kj`, `sing_store.py:171`) overrides the typed name on `/submit` and `/tip-claim`.
...
| Enable browser notifications | `maybeShowPushPrompt` `:3493`, `requestPushPermission` `:3409`, `ensurePushSubscription` `:3375` | POST `/sing/push/subscribe?t=` `{phone, device_id, singer_name, subscription:{endpoint, keys:{p256dh,auth}}}` | `sing.py:push_subscribe:1093` → `insert_push_subscription` (`sing_store.py:1228`, upsert on `(token,endpoint)`, clears `disabled_at`) | `sing_push_subscriptions` | Later pushes come from `PushDispatcher`. **Ladder** (`push_dispatcher.py:29`): after each rotation mutation, debounced 0.5 s, it scans active subs for the current token. Each sub maps to the first active entry whose linked `sing_requests` row has the sub's phone, or (phone-less singers) the sub's `device_id` (Night-scoped, `app.py` `_phone_for_rotation_entry` / `_device_for_rotation_entry`). Decision pushes match by phone or device too. Steps: `up_in_2` (position 3), `up_next` (positions 1–2), `now_singing`. Deduped via `last_sent_state`. **Decision** pushes: approve or reject from KJ routes (`routes.py:6487,6535`), and auto-resolve `resolved_alt` / `unavailable` (`routes.py:890`, `_notify_sing_outcome:5981`). Copy is at `push_dispatcher.py:67`. A 404 or 410 from the push service disables the sub. | Tok. **Phone or `device_id` is required** (400 if neither), and so are the name, endpoint, and keys. A phone, if given, must match `_PHONE_RE`. The client checks `resp.ok`; on failure the panel says browser notifications couldn't be turned on (`notify.browserFailed`) rather than claiming they're on. iOS outside standalone PWA mode gets an "Add to Home Screen" hint instead (`:3423-3425,:3525`). |
...
| Change photo consent later | `maybeShowPushPrompt` `:3562`, `savePhotoConsent` `:3592` | POST `/sing/photo-consent` `{consent, device_id, items}` | `sing.py:photo_consent:1832` | `singer_photo_consent` for each distinct Own+Night singer name | KJ rotation 📷 marker | RL-dev, token 403, consent must be `yes` or `no`. Zero owned requests → `updated: 0` (the choice goes out with the next submit instead). The UI rolls back on failure. Both buttons are disabled while a save is in flight, and re-tapping the current answer sends nothing. |
...
- `tests/e2e/` needs Playwright browsers installed (`playwright install`). It starts a real Flask server on :5099 (override with `KJ_E2E_PORT` when another worktree is running E2E, or the browser silently hits the other worktree's server) in a thread (`e2e/conftest.py:live_server`) with playback disabled and a file-backed `rotation.db`. The e2e tests are collected by a plain `pytest` run; there is no skip guard.
...
| D3 | `auto-deploy.sh` has no live-show guard | `auto-deploy.sh:29-41` restarts `kj-controller` on any `.py` diff | A merge mid-show restarts the app. Playback is **not** affected (players survive and are re-attached); the in-memory download queue is lost |
