...
| GET  | `/sing/my-requests` | **PUBLIC** — multi-id status feed (`?ids=1,2,3`, max 20) for the multi-song done screen. Optional `&name=` also returns queued entries naming that singer that none of their requests link to (KJ-added or partner-requested). These carry `entry_id` and `added_by_host`/`added_by` and can only be reordered |
