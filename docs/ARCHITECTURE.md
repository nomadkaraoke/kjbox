...
**Empty-state triage + make-request toggle (Phase C, 2026-04-23; reworked 2026-09-24):** When `/sing/search` returns `songs: []` and the query ≥ 3 chars, `renderEmptyStateTriage()` shows "We'll make it for you" (artist + title → `source_type=make`) and "Paste a YouTube link". The old make-it-yourself card (external link to gen.nomadkaraoke.com) was removed 2026-09-24. Under non-empty results a "Not the version you wanted? We can make it" link opens the same make card. The KJ controls the make card's visibility via a new `sing_accept_make_requests` meta flag (`rotation_meta` table, default on) exposed on `GET/POST /rotation/requests/config` as `accept_make_requests`. The same flag rides on `GET /sing/search` responses and the landing template dataset so the UI has it before the first search. `POST /sing/submit` enforces the flag server-side (400 `make_requests_disabled`) as defence-in-depth against stale clients.
...
- **Song + audio (`static-sing/make.js`):** gen's match-judge via `/sing/make/check` (fast pass in parallel with the search, full pass when needed; "Corrected to X — undo" / "Did you mean…?"), then `/sing/make/search` = quiet +1 show credit (`/api/kjbox/credits/show-credit`, idempotent per device/night/song — "free at the show") + gen `search-standalone`. Categories / best pick / confidence tiers are ported from gen's `audio-search-utils.ts`; YouTube-link fallback via gen `validate-url`. No audio edit, no private delivery, no customisation.
...
- `/sing/my-requests` adds `make: "making" | "needs_host"` (and drops the queue estimate) until the video is linked. Singer cancel doesn't cancel the gen job (they still get their video).
