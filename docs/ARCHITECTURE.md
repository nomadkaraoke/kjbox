...
| `gen_poller.py` | ~110 | `GenPoller` background thread: polls gen API for active jobs; on completion links ONLY the synced `NOMAD-####` master from the media index (`gen_status=syncing` until master-sync delivers it — no direct download); `complete` written only once linked |
| `make_jobs.py` | ~40 | `attach_on_approve`: binds a singer make-it's submit-time gen job to its new rotation entry |
| `sing_make.py` | ~330 | Singer make-it flow: `/sing/make/*` proxies gen **as the singer** (6-digit email code → gen session kept server-side in `sing_gen_accounts`; match-judge; show-credit top-up + audio search) and `create_job` (create-from-search / create-from-url at submit) |
...
| `sing.py` | ~2000 | Public `/sing/*` blueprint (landing, search, submit, status, rules, now, manifest, sw, push subscribe/unsubscribe, rename, forget, tip claims, event-info, preview delegates) + token-gate decorator + per-device/per-IP rate limiter + host-based route guard + QR-overlay auto-sync helper |
| `static-sing/i18n.js` + `static-sing/messages/*.json` | — | Singer-UI localisation runtime + message catalogues (`en.json` is the source; 32 more locales generated by `scripts/translate.py`). See docs/DEVELOPMENT.md § Singer UI localisation |
...
| POST | `/karaoke-nerds/search` | KN catalog search, server-composed via `unified_search(grouped=True, include_disc_only=True)` — returns `{songs, karaoke_nerds_timeout}`: the singer-search song grouping (local files + KN tracks with `local_path` / `divebar` mirror xref + standalone mirror files), plus disc-only KN rows for brands not held |
...
| POST | `/sing/submit` | **PUBLIC** — create a pending request (rate-limited per device via `device_id`, with a loose per-IP ceiling — see `_singer_rate_limited`). When `simple_mode` is on, the source allowlist is narrowed to `local`/`divebar`/`kn` and other types return 400 `simple_mode_disabled_source`. |
| GET  | `/sing/event-info` | **PUBLIC** — footer context for the singer UI: KJ display name, free-text footer message, switched-on notice keys (`notices`, in the KJ's order) + `notice_defs` (icon/text of the KJ's own `c-<id>` notices and their edits to pre-built ones), `social` links (instagram/facebook/tiktok/youtube/x/website under "Follow us"; email + phone under "Contact us" — the phone button opens 💬 Text (`sms:`) / 📞 Call (`tel:`)) and the `ask_photo_consent` flag (`footer_settings` from the Public Request Form modal) |
| POST | `/sing/photo-consent` | **PUBLIC** — singer changes their social-media photo/video consent (`yes`/`no`) for the singer names on the requests they prove ownership of (`items: [{id, edit_token}]`, like `/sing/update-phone`). `/sing/submit` also accepts an optional `photo_consent` field |
| POST | `/sing/preview/resolve` | **PUBLIC** — resolve a preview descriptor (`local`/`divebar`/`youtube` from search results, or `{"source":"entry","entry_id":N}` for a song already on the rotation — resolved server-side so file paths never reach the phone; 404 until the file is present) |
| GET  | `/sing/status/<id>` | **PUBLIC** — singer's own request status + rotation position |
| GET  | `/sing/my-requests` | **PUBLIC** — multi-id status feed (`?ids=1,2,3`, max 20) for the multi-song done screen. Optional `&device_id=` also returns queued entries naming a singer that device has submitted as tonight (or its alias) and that none of its requests link to (KJ-added or partner-requested). These carry `entry_id` and `added_by_host`/`added_by` and can only be reordered |
| GET  | `/rotation/singers/known` | KJ — tonight's singers `[{name, on_phone}]` for the add/edit existing-singer picker |
| GET  | `/sing/now` | **PUBLIC** — lightweight now-singing / up-next / queued-count for the landing widget |
| GET  | `/sing/rotation` | **PUBLIC** — full active rotation with cumulative wait estimates (`display_name` = every first name for duets, `entry_id` + `previewable` for the ▶ Preview button) |
...
**Approval dispatch:** `approve_sing_request(app, req, skip_download=False)` in `routes.py` creates a rotation entry based on `source_type` — `local` uses `rotation.add_entry(..., file_path=...)`, `divebar|youtube|kn` adds an entry + queues a download on the existing worker, `make` creates a `Being Made (!)` entry and attaches the request's submit-time gen job (or starts one via `GenClient` if none). `skip_download=True` (route reads it from JSON body) suppresses the download for the `divebar|youtube|kn` branch and creates an unlinked entry — used when the KJ has previewed the pasted YouTube URL and wants to attach a different file via the rotation 🔗 button. Auto-approve on submission calls the same helper inline (always `skip_download=False`).

**Grouped search + `kj_pick` (Phase A, 2026-04-23):** `GET /sing/search` returns `{songs: [{artist, title, version_count, in_library, versions: [...]}]}` — one group per unique `(artist, title)` after normalization (feat./paren/apos/punct/WS stripping). `_group_search_results` in `routes.py` does the collapsing (groups also carry standalone GCS-mirror files as KN-shaped versions with `kn.divebar` + `mirror_only`; file-quality tags like `(MPX)`/`[HM]` are ignored via `result_group_key`, grouping-only — `group_key` is also the persisted stats `song_key`); the KJ's KN panel uses the same grouping (`include_disc_only=True`), while rotation search still receives the flat shape via the kwarg default. The singer's "Auto-select best version" CTA (wording updated 2026-09-22; formerly "Let the KJ pick") submits `source_type="kj_pick"` with the full `versions[]` snapshot in `source_meta`. The admin approval route (`POST /rotation/requests/<id>/approve`) then accepts `{version_index}` — `_pick_version_from_kj_pick` translates the picked version into concrete `source_type/ref/meta` fields, `SingStore.update_request_source` writes them back, and `approve_sing_request` dispatches normally. With auto-approve ON, `resolve_kj_pick_best` binds the snapshot's highest-priority version (identical ranking to the admin picker's ⭐ BEST and the rotation-link search's gold "Best" pill — all three flow through `version_priority.rank_version` with the same config) before the same approval helper runs.
...
**Tip-for-heart + duet partner matching (2026-09-23):** `POST /sing/tip-claim` files a `source_type="tip"` meta-request (like `reorder`, never auto-approved) after the singer tips through a handle from `GET /sing/tip-info` (config: `sing_tip_venmo|cashapp|paypal|url`, `sing_tip_priority_threshold`). The admin approve route branch calls `apply_confirmed_tip` — hearts (`paid`) every active entry the singer appears in and, at/above the threshold, applies the singer-level `+1 priority_bias` (same as the rotation bump-up button). Duet partners: `GET /sing/singers` lists tonight's known singer names (KJ-typed "A & B" duet labels split into individuals) for the confirm screen's "👥 Existing singer" picker (v0.110.0 — replaced the always-visible tap-to-add chips), and `/sing/submit` canonicalizes typed partner names via `sing.match_known_singer` (folded-exact → unambiguous-first-name → length-scaled Damerau-Levenshtein) so rotation's exact-string singer identity doesn't sprout near-duplicates.

**Empty-state triage + make-request toggle (Phase C, 2026-04-23; reworked 2026-09-24):** When `/sing/search` returns `songs: []` and the query ≥ 3 chars, `renderEmptyStateTriage()` shows "Generate on demand" (artist + title → `source_type=make`; renamed from "We'll make it for you" 2026-09-26 so it doesn't sound like a person makes it) and "Paste a YouTube link". The old make-it-yourself card (external link to gen.nomadkaraoke.com) was removed 2026-09-24. Under non-empty results a "Not the version you wanted? Generate it on demand" link opens the same make card. The KJ controls the make card's visibility via a new `sing_accept_make_requests` meta flag (`rotation_meta` table, default on) exposed on `GET/POST /rotation/requests/config` as `accept_make_requests`. The same flag rides on `GET /sing/search` responses and the landing template dataset so the UI has it before the first search. `POST /sing/submit` enforces the flag server-side (400 `make_requests_disabled`) as defence-in-depth against stale clients.

**Singer make-it flow (2026-09-24/25):** karaoke-gen's customer job submission inside the singer UI, reusing gen's backend ([plan](archive/2026-09-24-singer-make-it-auto-flow-plan.md)).
- **Account:** the singer verifies their email with a 6-digit code (`/sing/make/send-code` → `/verify-code` → gen `POST /api/kjbox/auth/*`, partner secret `gen_kjbox_secret` → `X-Kjbox-Secret`, which also exempts venue sign-ups from gen's per-IP signup cap). The resulting gen **session token stays on the box** (`sing_gen_accounts`, per `device_id`); the phone only sees the email. A gen 401 later forgets it (re-verify).
- **Song + audio (`static-sing/make.js`):** gen's match-judge via `/sing/make/check` (fast pass in parallel with the search, full pass when needed; "Corrected to X — undo" / "Did you mean…?"), then `/sing/make/search` = quiet +1 show credit (`/api/kjbox/credits/show-credit`, idempotent per device/night/song — "free at the show") + gen `search-standalone`. Categories / best pick / confidence tiers are ported from gen's `audio-search-utils.ts`, and results are presented like gen's Choose Audio step (v0.118.0): hero card with the in-torrent **Filename**, Release, format + High/Medium/Low avail. badge, `[release_type / year / label / edition / media]`, "Wrong track?" / "Title match"; other options grouped by coloured category with counts, matching filenames sorted first, "+N more". YouTube-link fallback via gen `validate-url`. No audio edit, no private delivery, no customisation.
- **Submit:** `/sing/submit` (`source_type=make`, `source_meta={search_session_id, selection_index}` or `{youtube_url}`) creates the job on the singer's account (`create-from-search` / `create-from-url`, `X-Client-Id: kjbox`) BEFORE the request row exists, then the request skips the approval queue: the entry is added as **`Being Made (!)`** with the job attached (`make_jobs.attach_on_approve`). Per-device cap `sing_make_max_per_device` (3/night). A make can't be the target of a "change song".
- **Completion:** `GenPoller` links only the `NOMAD-#### - …` master once master-sync pulls it; `complete_gen_job` flips `Being Made (!)` → `Waiting` (never other statuses, never over a KJ's manual link). Lyrics review: the singer gets gen's review email (one-click sign-in link for kjbox jobs) and the KJ has the NEEDS REVIEW badge — first wins; an unreviewed song just stays at the bottom.
- `/sing/my-requests` adds `make` (and drops the queue estimate) until the video is linked: `making` → `review` (awaiting lyrics review) → `review_self` / `review_host` (review opened by the singer / by the KJ — gen `state_data.review_started_by == "admin"`, stored as rotation `gen_status` `in_review` / `host_review` by `map_gen_job`) → `rendering` (rendering + master-sync, "~10 minutes"); `needs_host` = audio pick/trim needed or the job failed. In the review phases My songs shows "**Tap here** to review it yourself": `POST /sing/make/review-link/<request_id>` (`{edit_token, device_id, locale}`) → gen `POST /api/kjbox/jobs/{job_id}/review-link` as the device's gen account → a one-click sign-in URL to that job's review (409 `not_in_review` once done). The KJ badge shows IN REVIEW for both in-review states. Singer cancel doesn't cancel the gen job (they still get their video).
...
**Editable venue notices + Contact us + search auto-correct + community folding (2026-09-26, v0.119.0):**
- **Notices:** `footer_settings.notice_defs` = `{key: {icon, text}}`, validated by `SingStore._clean_notice_defs`. A pre-built key (`FOOTER_NOTICE_KEYS`) may override its icon and/or text; blank text keeps the translated default, with its emoji unless the icon is overridden. KJ-authored notices are `c-<id>` keys (max 12) and need text; an emptied or deleted one is also removed from `notices`. `notices` stays the ordered list of switched-on keys (legacy list-of-keys configs load unchanged). KJ UI: an editable list with an on/off tick, emoji, wording, ↑/↓ and ✕ (own notices only), plus "+ Add notice". The highlighted "A note from your host" message is separate.
- **Contact us:** `social.phone` (`_PHONE_RE`, 7–15 digits, `tel:` prefix stripped). The singer footer splits into **Follow us** (left) and **Contact us** (right, email + phone). House rules (Rotation tab) now render directly under the rotation card, above the notices and socials: `#sing-rules-footer` comes before `#sing-event-footer` in `sing.html`.
- **Search auto-correct:** when `/sing/search` returns no songs, `sing.js` calls `GET /sing/search/resolve?q=` (token-gated, 20 per 5 min per device, cached per query). That calls gen `POST /api/kjbox/catalog/resolve` (partner secret; one Gemini call that splits the free text into artist/title and fixes typos). A confident cosmetic or content verdict re-runs `unified_search("<artist> <title>")`, and if that finds songs the singer sees them under "✓ Corrected to X — you typed "…". Undo" (Undo shows the typed search again, with its empty-state triage). An ambiguous verdict shows "Did you mean…?" chips instead. gen being offline or unconfigured changes nothing.
- **Community release already on the SSD:** `routes._fold_community_into_local` runs after `_suppress_mastered_kn_tracks` and generalises it from NOMAD to every brand. It drops a KN **community** YouTube track when exactly one local **video** file has the same normalised song key and the same `canonical_brand_for_match` (e.g. KN `KARAR` == `KARAR-093 - The Strokes - The Adults Are Talking.mp4`). The file gets `is_community` (so it ranks in the community tier instead of "unknown", which had put the YouTube copy first and re-downloaded it) and `alt_youtube_url`. Commercial tracks, mirror rows, CDGs and ambiguous matches never fold. At design time about 24k of 63k KN community rows had such a same-brand file on the SSD.
- **Song cards:** a collapsed multi-version card now shows the auto-pick's version, i.e. `versions[0]` in the compact row with Best, pills, brand, availability and a prominent ▶ Preview. "Searching…" is an animated row of colour-cycling notes, and becomes "Checking the spelling…" while resolving.

**Footer social links + social-media photo consent (2026-09-24, v0.112.0):** `footer_settings` (the `sing_footer_settings` `rotation_meta` blob) gained `social` (a whole-set-replace map over `SingStore.FOOTER_SOCIAL_KEYS`; URLs must be http(s) — a bare `instagram.com/x` gets `https://` prepended, any other scheme is a 400 — and `email` becomes a `mailto:` link) and `ask_photo_consent`. The singer footer renders the links as an icon row (inline simple-icons SVGs; `sing.js` re-checks the scheme before writing `href`). With `ask_photo_consent` on, the confirm step asks once per device ("OK to post photos/videos of you on social media?" — optional), the answer lives in `localStorage: sing_photo_consent` and rides on every `/sing/submit` as `photo_consent`, and My songs offers a change control (`POST /sing/photo-consent`, edit_token-gated). Storage is a `singer_photo_consent` table keyed on the folded singer **name** (rotation identity is the name string), read **night-scoped** (`updated_at >= night_started_at`, fails closed) so a same-name walk-in on a later night never inherits someone's choice. The device re-sends its choice with each night's first request. `routes._add_photo_consent` (part of `_decorate_rotation_entries`) attaches `photo_consent: {name: "yes"|"no"|null}` per entry — one key per duet partner — and the KJ rotation shows a 📷 marker beside each name (green = OK, red struck-through = no; since v0.112.1 unknown is treated as **no consent** — the same struck-through camera with a dashed outline). Clicking it toggles OK ⇄ no (unknown → OK) via `POST /rotation/singer/photo-consent {singer, consent}`. Renames/merges carry the choice (`carry_photo_consent`, called from `persist_rename` and the self-rename path); on a conflict **"no" wins**.
