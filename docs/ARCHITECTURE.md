...
| GET  | `/sing/event-info` | **PUBLIC** — footer context for the singer UI: KJ display name, free-text footer message, enabled pre-built notice keys, `social` links (instagram/facebook/tiktok/youtube/x/website/email) and the `ask_photo_consent` flag (`footer_settings` from the Public Request Form modal) |
