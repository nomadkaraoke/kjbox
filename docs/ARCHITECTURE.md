...
| `gen_poller.py` | ~150 | `GenPoller` background thread: polls gen API for active jobs. On completion it links the NOMAD-#### master pulled by master-sync (`gen_status=syncing` while waiting, up to `gen_master_wait_seconds`), else downloads gen's 720p directly (`source=gen` → `downloads/gen/`, 3 attempts); `complete` is only written once a file is linked |
| `make_jobs.py` | ~130 | Submit-time gen jobs for singer `make` requests: `submit_early` (background `create_job`, job id stored on the request) + `attach_on_approve` (lock-guarded hand-over to the rotation entry — never a duplicate job) |
...
**Fully-automatic make flow (2026-09-24):** a singer `make` request starts its karaoke-gen job **at submit time** (`make_jobs.submit_early` from `/sing/submit` and `/sing/requests/<id>/change`; `sing_requests.gen_job_id` + `gen_submit_state` = `submitting|submitted|failed`; a `submitting` row left by a dead process is marked `failed` on store open). Per-device cap per night: `sing_make_max_per_device` (default 3, 429 `make_limit`). Approval (manual or auto) creates the entry with status **`Being Made (!)`** (Auto Order pins it below singable songs) and `make_jobs.attach_on_approve` binds the early job; only with no early job does it fall back to `create_job` at approval. The `GenPoller` links the finished video — preferring the `NOMAD-#### - …` master that gen pushes to the Divebar bucket and master-sync pulls within ~60s — and `RotationManager.complete_gen_job` flips `Being Made (!)` → `Waiting` (never touches other statuses, never overwrites a KJ's manual link). `/sing/my-requests` adds `make: "making" | "needs_host"` for unfinished make requests (and drops their queue estimate); the KJ row shows MAKING / NEEDS INPUT / NEEDS REVIEW / RENDERING / SYNCING badges. Rejected or cancelled requests leave their gen job running (it still yields a catalog track).
