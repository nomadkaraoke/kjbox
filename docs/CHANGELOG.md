...
KJ side driven through the box API), then fixes for everything it surfaced, plus the
same auto-translation pipeline karaoke-gen uses. Findings + rationale:
`docs/archive/2026-09-23-singer-ux-walkthrough-i18n-plan.md`.

**Localisation (new)**
- `kj-controller/static-sing/messages/en.json` is the single source of every singer-facing
  string (~350 keys, nested, `{placeholder}` interpolation, CLDR plural forms). 32 locale files (33 languages with English) are generated by `scripts/translate.py` (copied from gen: Gemini/Vertex two-pass
  translate+review, shared GCS cache `nomadkaraoke-translation-cache`, `glossary.json`
  keeps brand/product names untranslated). `scripts/validate-translations.py` checks key +
  placeholder parity; `scripts/check-i18n-keys.py` checks every `t()` key in `sing.js`
  resolves. New `.github/workflows/i18n.yml` runs both; `.githooks/pre-commit` auto-translates
  when `en.json` is staged; workspace `scripts/translate-all.sh` gained the 6th directory.
- Runtime `static-sing/i18n.js` (no build step): locale = `?lang=` → saved choice →
  `navigator.languages` → English; loads `messages/<locale>.json` (English always loaded as
  fallback, so a missing string never renders as a raw key); sets `<html lang dir>` (RTL for
  ar/he); fills server-rendered strings via `data-i18n` attributes (offline banner, closed
  page, code entry). 🌐 language pill on every card opens a native-name picker; switching
  re-renders in place. Service worker shell now caches `i18n.js` + `en.json`.

**Walkthrough fixes (singer UI)**
- **Shared-venue-wifi rate limit** — all singer mutations were budgeted per IP (5 / 5 min),
  which on venue wifi is one address for the whole bar. Now budgeted per device
  (`sing_rate_limit_per_device`, default 8 / 5 min) with a loose per-IP ceiling
  (`sing_rate_limit_per_ip`, default now 60). Every 429 shows one honest "too many changes
  from this phone" message (reorder/rename used to say "couldn't save").
- **Search ranking** — grouped results are ranked by relevance (exact title → in library →
  version count) instead of raw catalog order, and the singer search's local-catalog cap is
  60 (`sing_search_catalog_limit`) instead of the KJ picker's 10, which used to truncate the
  popular songs entirely (Luis Fonsi 5th for "despacito"; Jeff Buckley 8th, Leonard Cohen
  10th for "hallelujah").
- **Single-version songs** now get the same decision row as multi-version ones (Community/
  Commercial pill, format pill, brand, ▶ Preview) — previously a bare "Add to queue".
- **One primary button everywhere**: "Request this song →" (multi-version songs add "Best
  version picked automatically · Choose from N versions →").
- **Change-song mode is visible**: a "Swapping out: <song>" banner on search (with "Keep my
  song" to back out), "Swap to this song?" confirm copy, no partner section on a swap, and
  the pending replacement card says "Replaces <song> once the host confirms"
  (`supersedes_request_id` now on the public request view).
- A song that is **playing right now** no longer offers ⇄ Change / ✕ Cancel.
- **Duets** show every first name in the rotation ("José & Maria") via new `display_name`.
- **Copy for everyone**: "KJ" → "the host" (one explanatory "your host (the KJ)" on the name
  screen), idioms removed ("break a leg", "flag the KJ down"…), "#3 in line", "You're next —
  right after this song" (My songs now agrees with the Rotation tab), empty-state numbering
  follows the cards actually shown, house rules quote the configured ♥ threshold and no longer
  reference a screen that doesn't exist.
- Name screen: Nomad Karaoke brand strip (+ "with <KJ name>" when set), phone example in the
  venue's SMS region, shorter consent text, search box focused after Next.
- Rotation rows wrap to two lines up to 480 px (titles no longer truncate on a 390 px phone);
  the next-song bar wraps its status instead of truncating; notifications block renders
  immediately (no 2 s layout shift); links inside it keep the accent colour; favicon 404 on the
  public host fixed (`static-sing/favicon.ico`).
- Unknown local disc-id prefixes are labelled "Library file EEK-01507" rather than shown as
  the *brand*.

**New (Andrew's mid-session to-do)**
- **▶ Preview on My songs cards and every Rotation-tab row** for songs already on the box:
  new `previewable` + `entry_id` fields on `/sing/rotation` and `/sing/my-requests`;
  `POST /sing/preview/resolve` accepts `{"source":"entry","entry_id":N}` and resolves the
  file server-side (paths never reach the phone; 404 until the file is present).
- **KJ-configurable footer** under every singer screen (Public Request Form modal → "📣
  Footer & venue notices"): a free-text message shown verbatim + pre-built, translated,
  toggleable notices (🔌 phone chargers at the host's table, 📺 lyrics on the big screen,
  🎤 duets welcome, 📱 add more songs any time, 💜 tips, 📶 wifi, 💧 water, 📸 photos).
  Stored as `sing_footer_settings` in `rotation_meta`; rides `GET/POST
  /rotation/requests/config` (`footer_settings`); singers read `GET /sing/event-info`.

**Tests**: `tests/test_sing_ux_i18n.py` (rate-limit keying, duet names, relevance ranking,
footer store/config/endpoint, entry preview, message-file parity + no-jargon guard) and
`tests/e2e/test_sing_i18n_footer.py` (language switch/persist/RTL/fallback, code-entry
translation, footer, single-version row, auto-pick hint, empty-state numbering, change-mode
banner + confirm, now-singing card, "replaces" line, duet rotation rows + preview).

**Noted, not changed**: Tip tab pre-selects the threshold amount; the 300 ms anti-mis-tap
window silently swallows a tap on freshly rendered results; singers can't see a YouTube
track still downloading on the box; a duet partner is flagged `has_tipped` in KJ singer
stats when only the lead's entry was hearted.
