...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if args.upload:
            from kjupload import upload_to_kjbox
            t = Timer.begin("upload to kjbox (nomadpc)")
            try:
                # Tag with a FASTGEN label — survives kjbox title normalisation, so
                # these low-res drafts are easy to spot and never confused with a
                # proper version.
                result = upload_to_kjbox(
                    out_path, label="FASTGEN",
                    note="Low-res on-the-fly draft generated by fastgen.",
                ...
                t.done()
                log(f"upload failed ({type(exc).__name__}: {exc}) — the mp4 is saved locally, upload it manually.")
                return 1
        return 0
    finally:
        if args.keep_temp:
